...
def new_rfq_customer(request):
    ...
    customer = request.POST.get('customer',False)
    if customer:
        customer = ChartOfAccount.objects.get(account_title = customer)
        customer = ChartOfAccount.objects.filter(id = customer.id)
        row = serializers.serialize('json', customer)
...
def new_quotation_customer(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for value in items:
                item_id = Add_products.objects.get(id = value["id"])
                quotation_detail = QuotationDetailCustomer(serial_no = value["serial_no"], item_id = item_id, quantity = value["quantity"], unit_price = value["unit_price"], remarks = value["remarks"], quotation_id = header_id)
...
def edit_quotation_customer(request,pk):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            quotation_detail_update = QuotationDetailCustomer(serial_no = value["serial_no"],item_id = item_id, quantity = value["quantity"], unit_price = value["unit_price"], remarks = value["remarks"], quotation_id = header_id)
...
def print_dc_customer(request,pk):
    ...
    pdf = render_to_pdf('customer/dc_customer_pdf.html', {'company_info':company_info,'image':image,'header':header, 'detail':detail,'po_client':po_client})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
...
def mrn_customer(request):
    cursor = connection.cursor()
    all_dc = cursor.execute('''select DCH.id, DCH.dc_no, COA.account_title, DCH.date, DCD.quantity ,DCD.accepted_quantity from customer_dcheadercustomer DCH
                                inner join transaction_chartofaccount COA on DCH.account_id_id = COA.id
                                inner join customer_dcdetailcustomer DCD on DCD.dc_id_id = DCH.id
                                group by DCH.dc_no''')
