...
def chart_of_account(request):
    ...
    if request.method == 'POST':
        ...
        prc_basis = request.POST.get('prc_basis')
        ...
        coa = ChartOfAccount(account_title = account_title, parent_id = account_type, opening_balance = opening_balance, phone_no = phone_no, email_address = email_address, ntn = ntn, stn = stn, cnic = cnic ,Address = address, remarks = remarks, credit_limit=credit_limits, attn = attn, prc_basis = prc_basis, lead_time = lead_time, validity = validity, payment = payment, currency = currency , exchange_rate = exchange_rate)
        coa.save()
...
def sales_tax_invoice(request,pk):
    ...
    detail = cursor.execute('''Select SaleId,DcNo,po_no,product_name, product_desc, unit, quantity, cost_price, sales_tax, Hs_Code from(
                            ...
                            DC.dc_no as DcNo, PO.PO_no  as po_no, SD.hs_code as Hs_Code
    ...
    dc_no = cursor.execute('''select DHC.dc_no from customer_dcdetailcustomer DC
                            inner join customer_dcheadercustomer DHC on DHC.id = DC.dc_id_id
                            inner join transaction_saledetail SD on DHC.id = SD.dc_ref
                            where SD.sale_id_id = %s
                            group by DHC.dc_no''',[pk])
    dc_no = dc_no.fetchall()
    dc_no = [x[0] for x in dc_no]
    print(dc_no)
    pdf = render_to_pdf('transaction/sales_tax_invoice_pdf_lines.html', {'company_info':company_info,'image':image,'header':header, 'detail':detail,'total_amount':total_amount,'total_amount':total_amount,'dc_no':dc_no})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
...
def commercial_invoice(request,pk):
    ...
    pdf = render_to_pdf('transaction/commercial_invoice_pdf_lines.html', {'company_info':company_info,'image':image,'header':header, 'detail':detail,'total_amount':total_amount,'total_amount':total_amount,'dc_no':dc_no,'po_client':po_client})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "SaleTaxInvoice%s.pdf" %(header.sale_no)
        content = "inline; filename='%s'" %(filename)
